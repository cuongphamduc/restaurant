...
const Pagination = ({
  ...
  const listNumberShow = [5, 10, 20, 50];
  ...
  useEffect(() => {
    ...
    if (numberOfPages.length <= 4 && !showTotal) {
    ...
    } else if (numberOfPages.length <= 8 && showTotal) {
      tempNumberOfPage = numberOfPages;
      ...
      const slice = numberOfPages.slice(
  ...
  function handleChangeNumberShow(_name, value) {
    ...
    const newTotalPage =
      numberTotalPage > numberTotalPageFixed
        ? numberTotalPageFixed + 1
        : numberTotalPageFixed;
  ...
  return (
    <div className="paginition-container" ref={refContainer}>
      <div
        className={`info ${
          showTotal ? null : 'paginition-container__info-view-small'
        }`}
      >
        {showTotal && (
      ...
      <div
        className={`handle-box ${
          showTotal ? null : 'paginition-container__handle-box-view-small'
        }`}
      >
        <div className="buttons">
          ...
          {arrOfCurrentPage.map((pageItem, index) => {
            return (
              <button
                ...
                key={index}
                className={
                  'button' + (pageItem === currentPage ? ' button-active' : '')
                }
          ...
          <button
            type="button"
            disabled={
              currentPage === totalPage || totalPage === 0 ? true : false
            }
