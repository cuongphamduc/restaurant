...
const UpdateForm = (props) => {
  ...
  const handleSubmit = (values) => {
    ...
    (async () => {
      try {
        const { data } = await billApi.update({
  ...
  const handleSelectCustomer = (data) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.log("customer", lishDish, dish)

            if(lishDish.length == 0){
            for(let i = 0; i < dish.length; i++){
                lishDish.push({
                    dish: dish[i],
                    number: dish[i].soluong
                })
            }}
            ...
            setTypePay(bill.kieuthanhtoan)
            setTypeBox(bill.loaihopcom)
            form.setValue("suatan", bill.suatan)
            setNote(bill.ghichu)
        } catch (error) {
            console.log('Failed to fetch customer list: ', error);
        }
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)}>
      <Modal
        ...
        <div id="create-form-bill-container" className='create-form-bill-container' onKeyDown={handleShorcutCreateBill} tabIndex={"0"}>
            ...
            {lishDish.length == 0 && <i id="not_selected_dish">Chưa chọn món ăn</i>}
            <button
                type='button'
                className='button-add'
                onClick={() => {
            ...
            <div className="create-form-bill-container__line">
              ...
              <div className="create-form-bill-container__line__radio">
                <input onChange={handleChangeTypeBox} type="radio" id="khayinox" name="hopcom" value="0" checked={typeBox==0} />
                <label for="khayinox">Khay inox</label>
              </div>

              <div className="create-form-bill-container__line__radio">
                <input onChange={handleChangeTypeBox} type="radio" id="hopnhua" name="hopcom" value="1" checked={typeBox==1}/>
